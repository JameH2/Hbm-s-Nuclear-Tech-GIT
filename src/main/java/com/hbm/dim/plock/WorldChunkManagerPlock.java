...
import com.hbm.dim.plock.GenLayerPlock.GenLayerPlock;
...
public class WorldChunkManagerPlock extends WorldChunkManager
{
	...
	protected WorldChunkManagerPlock()
	{
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList();
		this.myBiomesToSpawnIn.add(BiomeGenBasePlock.plockPlains);
		this.myBiomesToSpawnIn.add(BiomeGenBasePlock.plockLowlands);
	...
	public WorldChunkManagerPlock(long seed, WorldType type)
	{
		this();
		GenLayer[] genLayers = GenLayerPlock.makeTheWorld(seed);
		this.unzoomedBiomes = genLayers[0];
		this.zoomedBiomes = genLayers[1];
	}

	public WorldChunkManagerPlock(World world)
	{
	...
	public BiomeGenBase getBiomeGenAt(int par1, int par2)
	{
		...
		if (biome == null)
		{
			return BiomeGenBasePlock.plockPlains;
		}
		return biome;
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width)
	{
		...
		for (int i = 0; i < length * width; i++)
		{
			...
			{
				par1ArrayOfBiomeGenBase[i] = BiomeGenBasePlock.plockPlains;
			}
		}
